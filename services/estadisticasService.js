...
const mapearSemana = (filas) => {
  const porDia = new Map(filas.map((fila) => [fila.dia, Number(fila.total) || 0]));
...
const mapearMeses = (filas) => {
  const meses = new Map();

  filas.forEach((fila) => {
    if (!meses.has(fila.mes)) {
      meses.set(fila.mes, {
        nombre: fila.mes,
        datos: [
    ...
    const indice = Number(fila.semana) - 1;

    if (indice >= 0 && indice < 4) {
      meses.get(fila.mes).datos[indice] = {
        semana: `S${fila.semana}`,
...
const generarEstadisticasUsuario = async (idUsuario) => {
  const resumenQuery = `
    SELECT
